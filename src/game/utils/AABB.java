...
public class AABB {
	...
	public void setWidth(float width) { w = width; }
	public void setHeight(float height) { h = height; }
	...
	public boolean collides(AABB bBox) {
		...
		if(Math.abs(ax - bx) < (this.w / 2) + (bBox.getWidth() / 2)) {
			if(Math.abs(ay - by) < (this.h /2) + (bBox.getHeight() / 2)) { return true; }
		}
	...
	public boolean colCircleBox(AABB aBox) {
		...
		if(Math.sqrt(dx * dx + dy * dy) < (r / 2)) { return true; }
